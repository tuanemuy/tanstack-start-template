...
- **Domain Layer** (`app/core/domain/`): Pure business logic, types, and port interfaces.
    ...
    - `app/core/domain/${domain}/ports/**.ts` — Port interfaces for repositories / external APIs. The corresponding repository slot is added directly to `UnitOfWorkContext` in `app/core/application/execution/unitOfWork.ts` (see "Unit of Work" below).
    ...
    - `app/core/domain/common/pagination.ts` — Pure types only: `Pagination`, `PaginationResult<T>`. No Zod schemas — runtime validators live at the application boundary that consumes them (e.g. inline in `listTodos.ts`).
...
- **Application Layer** (`app/core/application/`): Use cases and orchestration.
    ...
    - `app/core/application/ports/outboxRepository.ts` — `OutboxRepository` port (`save` / `listPending` / `markProcessed` / `pruneProcessed`) used by the outbox pattern. Lives in the application layer (NOT the domain) because the outbox is an infrastructural delivery mechanism, not a domain concept; domain code never imports it.
    ...
    - `app/core/application/workers/outboxPrune.ts` — `pruneOutbox(container, { retentionMs })` deletes processed outbox rows older than the cutoff. Schedule on a cron / queue runner sized to your retention policy; the template ships the utility but not the schedule.
...
- **Shared lib** (`app/lib/`)
    ...
    - `app/lib/error.ts` — `AnyError` base, `fromUnknown`, `formatErrorMessage`, plus the abstract `CodedError<TCode extends string>` base shared by `SystemError`, `ApplicationError`, and `BusinessRuleError`. `CodedError` owns the typed `code` field, the default `retryable: false` getter, and the abstract `toSerialized()` contract; concrete subclasses pin the discriminant `kind`. Living in `app/lib/` is what lets every layer (adapter / application / domain) extend the same base without violating the hexagonal direction. Sits below every layer in the dependency graph.
    - `app/lib/systemError.ts` — `SystemError` (low-level failures: DB driver, network, storage). Extends `CodedError<SystemErrorCode>`. Lives here rather than under `core/application/errors/` so adapter code can import it without reaching upward into the application layer (hexagonal: adapters depend on shared lib + domain ports, not on application modules). Re-exported from `core/application/errors` so existing application-side call sites do not need to know about the new path.
...
- `UnitOfWorkContext` directly enumerates every domain repository the callback can touch plus `collectEvents(events)`. Adding a new domain is a one-line edit here (and a wiring edit in `app/core/adapters/drizzleSqlite/unitOfWork.ts` to instantiate the repository inside the transaction). At template scale this is cheaper and more readable than declaration-merging from each domain's port file.
...
- Each domain owns its event union (e.g. `app/core/domain/todo/events.ts`) and a per-event-type decoder map (e.g. `todoEventDecoders: Record<TodoEvent["type"], EventDecoder<TodoEvent>>`). Each entry validates the wire payload strictly and re-runs every branded field through its value-object factory. **Decoders throw on a malformed row** — the relay worker catches per-row so one bad row does not abort the whole batch. A thin `decode*Event(type, payload, meta)` dispatcher is also exported for ad-hoc / test use, but the worker registry consumes the per-event map directly so adding a new variant without a decoder fails at compile time.
...
- `app/core/application/ports/outboxRepository.ts` defines a single `OutboxRepository` port (application-layer, not domain — the outbox is an infrastructural delivery mechanism):
    ...
    - `pruneProcessed(olderThan)` — delete already-processed rows whose `processed_at` is strictly older than the cutoff, returning `{ deleted }`. Pending rows are never touched.
...
- `app/core/application/workers/eventRelayWorker.ts` polls `listPending`, decodes each entry via a registry keyed on the **full** `event.type` string, dispatches via `Promise.allSettled`, logs decode/dispatch failures via `container.logger.error`, and calls `markProcessed` for the successfully dispatched ids only. The default registry is built by spreading every domain's per-event decoder map (e.g. `{ ...todoEventDecoders }`); each map is typed `Record<DomainEvent["type"], EventDecoder<DomainEvent>>`, so adding a new event variant without registering its decoder is a `pnpm typecheck` failure rather than a runtime "no decoder" log line.
- `app/core/application/workers/outboxPrune.ts` exposes `pruneOutbox(container, { retentionMs })` — computes `cutoff = clock.now() - retentionMs`, calls `outboxRepository.pruneProcessed`, and logs the result. Raw milliseconds is the canonical unit; callers compose `days * 86_400_000` etc. at the call site. Safe to run concurrently with the relay worker — a row only becomes a prune candidate after `markProcessed` has stamped it.
...
- Rows with `processed_at IS NOT NULL` are retained for audit/debugging. The template ships `pruneOutbox` but **does not schedule it** — you choose the retention window and runner (cron / queue worker) before going to production.
...
- `app/core/domain/error.ts` defines `BusinessRuleError<TCode extends string = never>`, a `CodedError` subclass. It carries its own `toSerialized()` method that returns a `SerializedError` envelope, so the presentation layer never needs to special-case `instanceof BusinessRuleError`. The `TCode` default is intentionally `never` (not `string`) — every throw site must narrow to its domain's literal-union code type (e.g. `BusinessRuleError<TodoErrorCode>`); `isBusinessRuleError(...)` narrows to `BusinessRuleError<string>` for the generic catch case.
...
- `app/core/application/errors/index.ts` defines:
    - The abstract `ApplicationError<TCode>` base (a thin alias over `CodedError`) plus `NotFoundError`, `ConflictError`, `UnauthenticatedError`, `ForbiddenError`, `ValidationError`. `SystemError` is defined in `@/lib/systemError` (so adapters can import it without depending on the application layer) and re-exported here for application-side call sites.
- Each subclass exposes a `retryable` getter (defaulting to `false` from `CodedError`). `SystemError` returns `true` for transient codes (`NetworkError`, `ExternalApiError`); `ConflictError` returns `false` because retry safety depends on the command.
