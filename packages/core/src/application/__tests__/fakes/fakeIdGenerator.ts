 ...
 * are required for `parse` to accept the output — the regex enforces
...
export class FakeIdGenerator implements IdGenerator {
  ...
  next(): GeneratedId {
    ...
    return `ffffffff-ffff-7fff-8fff-${tail}` as GeneratedId;
  }

  parse(raw: string): GeneratedId | null {
    return UUID_V7_PATTERN.test(raw) ? (raw as GeneratedId) : null;
  }
